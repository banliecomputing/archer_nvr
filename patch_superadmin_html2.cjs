            ...
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:1.5rem; flex-wrap:wrap; gap:1rem;">
                ...
                <div style="width:300px; padding-left:1.5rem; border-left:1px solid var(--border);">
                    ...
                    <p style="font-size:0.75rem; color:var(--text-muted); margin-bottom:1rem;">
                        Tindakan ini akan mengembalikan seluruh pengaturan database (User, Kamera, Storage) kembali ke kondisi standar pabrik (default). Rekaman MP4 di penyimpanan tidak akan dihapus.
                    </p>
                    <button id="saBtnFactoryReset" class="btn btn-primary" style="background:#ef4444; width:100%; font-size:0.85rem; border-color:#ef4444;">
...
// Insert it right before the last closing </div> that wraps the saDashboard content
html = html.replace(/<\/div>\s*<\/div>\s*<script src="superadmin\.js"><\/script>/, '</div>\n' + newSection + '    </div>\n    <script src="superadmin.js"></script>');

fs.writeFileSync('public/superadmin.html', html);
console.log('Patched superadmin.html again');
